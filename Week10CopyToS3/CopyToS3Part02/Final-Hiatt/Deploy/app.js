...
app.get('/exportFromDatabase', function(request, response){
	console.log('/exportFromDatabase SERVER called');
	console.log(request.query);
	console.log(JSON.stringify(request.query.exportObject));
	
	var fileStringContent = JSON.stringify(request.query.exportObject.content);
	var folderPathFile = request.query.exportObject.folderPath +"/"+request.query.exportObject.fileName;
	
	// save file content to folderPath
	fs.writeFile(folderPathFile, fileStringContent, 'utf8', function(err) {
		if(err){
			throw err;
		}else{
			console.log("Successfully wrote file to: " + folderPathFile);
		}
	});
});
...
app.get('/insertFile', function(request, response) { 'use strict';
	console.log("/insertFile called Server side");
	console.log("REQUEST.QUERY.INSERTOBJECT: ");
	console.log("=======================");
	console.log(request.query.insertObject);
	// get folder path + file name
	var folderPathFile = request.query.insertObject.folderPath+"/"+request.query.insertObject.fileName;
	// get file contents
	var fileContent = fs.readFileSync(folderPathFile, 'utf8', function(err) {
		if(err){
			throw err;
		}else{
			console.log("Successfully read: "+folderPathFile);
		}
	});
	// put file contents in object
	request.query.insertObject.content = fileContent;
	// call mongo to insert object
	queryMongo.insertIntoCollection(response, request.query.insertObject);
...
/* Not getting home directory -- assuming on linux box
...
app.get('/walk', function(request, response) {
	...
	*/
	...
	console.log(request.query.uploadDir);
	var dirToWalk = request.query.uploadDir;
	var filePattern = request.query.fileType;
	console.log("About to walk: " + dirToWalk);
	//walk(dirToWalk, ['Sonnet01.md'], ['node_modules', 'JavaScript'], function(err, data) {
	walk(dirToWalk, filePattern, ['node_modules', 'JavaScript'], function(err, data) {
		if (err) {
			console.log(err);
			response.send({
				result : "Error",
